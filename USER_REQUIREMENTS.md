...
- **已实现能力（以代码为准）**：
  ...
  - **相册页**（[`lib/gallery/gallery_page.dart`](lib/gallery/gallery_page.dart)）：分页加载（每页 120 条）、接近列表底部自动加载更多、下拉刷新；按资源**创建时间**排序，AppBar 可在**日期倒序（最新在前）**与**日期正序（最旧在前）**之间切换；监听媒体库变更后刷新列表。
